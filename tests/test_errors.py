...
class TestDrifterError:
    def test_base_class_attributes(self) -> None:
        classification = Classification(
            action="block", reason="test", matched_pattern="git commit"
        )
        exc = DrifterError(
            "git commit", "git commit", "blocked by policy", classification
        )
...
class TestDangerousCommandError:
    ...
    def test_is_instance_of_drifter_error(self) -> None:
        ...
        exc = DangerousCommandError(
            "rm -rf /", "rm -rf /", "Blocked dangerous command", classification
        )
...
class TestApprovalRequiredError:
    def test_stores_command(self) -> None:
        classification = Classification(
            action="approval_required", reason="test", matched_pattern="git add"
        )
        exc = ApprovalRequiredError(
            "git add file.py", "git add", "requires approval", classification
        )
    ...
    def test_is_instance_of_drifter_error(self) -> None:
        ...
        exc = ApprovalRequiredError(
            "git add", "git add", "requires approval", classification
        )
        assert isinstance(exc, DrifterError)
