...
def _validate_single(md_file: Path, root: Path) -> list[DocIssue]:
    ...
    if not frontmatter_match:
        issues.append(
            DocIssue(file=rel_path, detail="Missing YAML frontmatter", severity="error")
        )
        return issues
    ...
    if "type" not in frontmatter:
        issues.append(
            DocIssue(
                file=rel_path, detail="Missing 'type:' in frontmatter", severity="error"
            )
        )
    else:
    ...
    for field in ("title", "created"):
        if field not in frontmatter:
            issues.append(
                DocIssue(
                    file=rel_path,
                    detail=f"Missing '{field}:' in frontmatter",
                    severity="warn",
                )
            )
